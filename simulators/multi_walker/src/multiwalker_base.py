...
class MultiWalkerEnv:
    ...
    def setup(self):
        ...
        self.walkers = [
            ...
            for index, sx in enumerate(self.start_x)
        ]
        ...
        self.action_space = spaces.Dict({
            "walker_{}".format(i): self.action_space[i] for i in range(self.n_walkers)
        })
    ...
    def reset(self, *, seed=None, options=None):
        ...
        return [self.observe(0), self.observe(1), self.observe(2)], {}
    ...
    def step(self, action):
        for i, (walker_name, action) in enumerate(action.items()):
            mapped = self.map[walker_name]
            is_last = i == self.n_walkers - 1

            self.step_walker(action, agent_id=mapped, is_last=is_last)
        obs = [self.observe(i) for i in range(self.n_walkers)]
        done = any(self.last_dones)
        average_reward = np.mean(self.last_rewards)
        return obs, average_reward, done, done, {}

    def step_walker(self, action, agent_id, is_last):
