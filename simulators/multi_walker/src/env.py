...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        ...
        for i, (walker_namne, walker) in enumerate(self.env.walkers.items()):
        ...
        if self.env.package == contact.fixtureA.body:
            if contact.fixtureB.body not in [walker.hull for (walker_idx, walker) in self.env.walkers.items()]:
                self.env.game_over = True
        if self.env.package == contact.fixtureB.body:
            if contact.fixtureA.body not in [walker.hull for (walker_idx, walker) in self.env.walkers.items()]:
                self.env.game_over = True

            #    self.env.game_over = True
        for walker_name, walker in self.env.walkers.items():
            for leg in [walker.legs[1], walker.legs[3]]:
                if leg in [contact.fixtureA.body, contact.fixtureB.body]:
                    leg.ground_contact = True

    def EndContact(self, contact):
        for walker_idx, walker in self.env.walkers.items():
            for leg in [walker.legs[1], walker.legs[3]]:
                if leg in [contact.fixtureA.body, contact.fixtureB.body]:
...
class MultiWalkerEnv(gym.Env, EzPickle):
    ...
    def __init__(self, n_walkers=3, position_noise=1e-3, angle_noise=1e-3, reward_mech='local',
        ...
        self.n_walkers = 3
        self.walkers = {
            f"walker_{i}": None for i in range(self.n_walkers)
        }
        ...
        for index, walker_idx in enumerate(self.walkers.keys()):
            self.walkers[walker_idx] = BipedalWalker(self.world, init_x=self.start_x[index], init_y=init_y, one_hot=self.one_hot)

        self.observation_space_info = {}
        for walker_idx in self.walkers.keys():
            self.observation_space_info[walker_idx] = self.walkers[walker_idx].observation_space

        self.action_space_info = {}
        for walker_idx in self.walkers.keys():
            self.action_space_info[walker_idx] = self.walkers[walker_idx].action_space

        self.observation_space_info = spaces.Dict(self.observation_space_info)
        self.action_space_info = spaces.Dict(self.action_space_info)
        self.setup()
    ...
    def _destroy(self):
        ...
        for walker_idx, walker in self.walkers.items():
            walker._destroy()
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.fallen_walkers = np.zeros(self.n_walkers, dtype=np.bool_)
        self.prev_shaping = np.zeros(self.n_walkers)
        self.prev_package_shaping = 0.0
        self.scroll = 0.0
        self.lidar_render = 0
        ...
        actions = {}
        for walker_idx in self.walkers.keys():
            actions[walker_idx] = np.array([0, 0, 0, 0])
        return self.step(actions, reset=True)

    def step(self, actions, reset=False):
        for walker_idx, walker_action in actions.items():
            self.walkers[walker_idx].apply_action(walker_action)

        self.world.Step(1.0 / FPS, 6 * 30, 2 * 30)

        obs  = {}
        for walker_idx, walker in self.walkers.items():
            obs[walker_idx] = walker.get_observation()
        ...
        for i, (walker_name, walker) in enumerate(self.walkers.items()):
            pos = self.walkers[walker_name].hull.position
            x, y = pos.x, pos.y
            xpos[i] = x

            wobs = self.walkers[walker_name].get_observation()
            nobs = []
            for j in [i - 1, i + 1]:
                ...
                if j < 0 or j == self.n_walkers:
                    ...
                    xm = (self.walkers[walker_name].hull.position.x - x) / self.package_length
                    ym = (self.walkers[f"walker_{j}"].hull.position.y - y) / self.package_length
            ...
            obs[walker_name] = np.array(wobs + nobs)
